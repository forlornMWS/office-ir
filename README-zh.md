...
## 作为 skill 安装(skills.sh)

office-ir 以可安装的 agent skill 形式发布在 [skills.sh](https://skills.sh)。一条命令即可安装整个 skill 文件夹——`SKILL.md` 以及预构建的 `office-ir.jar`:

```bash
npx skills add forlornMWS/office-ir --full-depth
```

安装后 skill 文件夹包含 `SKILL.md` 和预构建的 `office-ir.jar`,**无需 clone / 构建**即可使用,只需消费机器上有 JDK 17+ 运行时。skill 契约见 [`skills/office-ir/SKILL.md`](./skills/office-ir/SKILL.md),发布细节见 [`skills/office-ir/PUBLISHING.md`](./skills/office-ir/PUBLISHING.md)。
