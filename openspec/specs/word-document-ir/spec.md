...
定义 `.docx` → `WordIr` 的形状与提取:密封块树(body/sections)、run、命名样式表(解析继承)、编号、表格单元、节/页面布局、页眉页脚、图片、文档属性。仅 `.docx`(XWPF)。
...
> 实现注:v1/v2 中 `gridSpan`/`vMerge`/`shading` 因 POI 5.2.5 `CTTcPr` 无对应 getter 而暂为 null,留 v3(XmlCursor);此需求为契约目标。
...
> 实现注:v2 解析文档末 sectPr 的 pgSz(纸张/方向)+ pgMar(页边距,英寸);多节/页眉页脚内容留 v3。
...
> 实现注:页眉页脚内容解析留 v3;v1/v2 中 headers/footers 为 null。
